...
configBuilder.prototype.build = function(env) {
    ...
    // parse all self-referencing fields
    config = deepParseSelfReferences(config);
...
function deepParse(obj) {
    ...
    return Object.fromEntries(Object.entries(obj).map(([key, item]) => [
        key,
        typeof item === 'object' ? deepParse(item) : interpolate(item)
    ]));
}

function interpolate(string) {
    const fieldName = string.toString().split(":");

    const refType = fieldName[0];
    const refVar = (fieldName.length > 1) ? fieldName[1] : undefined;
    const defValue = (fieldName.length > 2) ? fieldName[2] : undefined;

    if (refType === "$env") {
        return refVar ? (process.env[refVar] || defValue) : "";
    }
    return string;
}
...
function findCfgReferences(config, currentPath = [], result = []) {
    if (Array.isArray(config)) {
        config.forEach((item, idx) => {
            findCfgReferences(item, currentPath.concat(idx), result);
        });
    } else if (config && typeof config === 'object') {
        Object.keys(config).forEach(key => {
            findCfgReferences(config[key], currentPath.concat(key), result);
        });
    } else if (typeof config === 'string' && config.startsWith('$cfg:')) {
        result.push({
...
function deepParseSelfReferences(config) {
    // find all self-referencing fields, if none found
    // return the config as is
    const references = findCfgReferences(config);
    if(references.length === 0) return config;

    // resolve all references
    references.forEach(ref => {
        ref.value = getValueByDottedPath(config, ref.ref);
        if (ref.value === undefined) {
            throw new Error(`Unable to resolve $cfg reference: '${ref.ref}'`);
        }
    });

    // apply resolved references to config
    references.forEach(ref => {
        ...
        target[keys[keys.length - 1]] = ref.value;
    });

    return config;
}

function getValueByDottedPath(obj, path) {
    if (!path) return obj;
